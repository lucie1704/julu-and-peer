<template>
  ...
  <div class="text-gray-500 text-lg mb-4">Free up !!!</div>
  <div class="text-3xl font-bold mb-4">Update your password</div>
  <form @submit.prevent="submitForm">

    <div class="mb-5">
      <label for="current-password" class="block mb-2 text-sm font-medium text-gray-900">Current password</label>
      <input id="current-password" type="password" v-model="currentPassword" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required />
      <small class="error" v-if="currentPasswordError">{{ currentPasswordError }}</small>
    </div>
...
import { passwordSchema } from '../../validation/updatePassword';
import { currentPasswordSchema } from '../../validation/updatePassword';
