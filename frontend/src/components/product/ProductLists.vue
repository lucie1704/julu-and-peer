<template>
  <div class="bg-white">
    <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
      <h2 class="text-2xl font-bold tracking-tight text-gray-900">Product Lists</h2>

      <div class="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
        <div v-for="product in products" :key="product.id" class="group relative">
          <router-link class="btn" :to="{ name: 'product-detail', params: { id: product.id }}" >
              ...
              <div class="mt-4 flex justify-between">
                <div>
                  ...
                  <p class="mt-1 text-sm text-gray-500">{{ product.ProductArtist.name }} - {{ product.ProductFormat.name }} - {{ product.ProductGenre.name }}</p>
                  <div class="flex items-center">
                    <svg v-for="star in 5" :key="star" class="h-5 w-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div>
                  <p class="text-sm font-medium text-gray-900">
                    <span v-if="product.discount > 0" class="line-through text-gray-500 mr-2">{{ product.price }}</span>
