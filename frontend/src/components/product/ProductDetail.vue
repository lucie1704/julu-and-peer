                  ...
                  <div class="sm:col-span-8 lg:col-span-7">
                    ...
                    <section aria-labelledby="information-heading" class="mt-2">
                      ...
                      <div class="mt-6">
                        ...
                        <div class="flex items-center">
                          <div class="flex items-center">
                            <StarIcon v-for="index in 5" :key="index" :class="[index <= rating ? 'text-gray-900' : 'text-gray-200', 'h-5 w-5 flex-shrink-0']" aria-hidden="true" />
                          </div>
                    ...
                    <section aria-labelledby="options-heading" class="mt-10">
                      ...
                      <form @submit.prevent="submitForm">
                        <div class="text-sm my-4 font-medium text-gray-900">Available: {{ product.availableStock }}</div>
                          <div class="flex flex-1 items-end justify-between text-sm">
                            <select v-model="quantity" class="m-1 p-1 block rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                              <option v-for="n in 10" :key="n" :value="n">{{ n }}</option>
                            </select>
                          </div>
                        <fieldset class="mt-10" aria-label="Choose a size">
                          <div class="flex items-center justify-between">
                            <div class="text-sm font-medium text-gray-900">Format</div>
                          </div>

                          <RadioGroup class="mt-4 grid grid-cols-4 gap-4">
                            <RadioGroupOption as="template" v-for="format in formats" :key="format.name" :value="format" :disabled="!format.inStock" v-slot="{ active, checked }">
                              <div :class="[format.inStock ? 'cursor-pointer bg-white text-gray-900 shadow-sm' : 'cursor-not-allowed bg-gray-50 text-gray-200', active ? 'ring-2 ring-indigo-500' : '', 'group relative flex items-center justify-center rounded-md border py-3 px-4 text-sm font-medium uppercase hover:bg-gray-50 focus:outline-none sm:flex-1']">
                                ...
                                <span v-if="format.inStock" :class="[active ? 'border' : 'border-2', checked ? 'border-indigo-500' : 'border-transparent', 'pointer-events-none absolute -inset-px rounded-md']" aria-hidden="true" />
