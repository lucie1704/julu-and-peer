...
import { OrderI, OrdersI, PlaceOrderI } from '../dto/order';

const ROOT_URL = 'http://localhost:3000/api/v1/customerorder';

interface OrderAPI {
  orderConfirm: (jwt_token: string, orderId: string) => Promise<string | null>;
  placeOrder: (jwt_token: string, data: PlaceOrderI) => Promise<{ message: string, orderId: string } | null>;
  getOrders: (jwt_token: string, customerId: string, status: string) => Promise<OrdersI | null>;
  getOrderDetails: (jwt_token: string, orderId: string) => Promise<OrderI | null>;
}
...
const orderAPI: OrderAPI = {
  ...
  async getOrders(jwt_token: string, customerId: string, status: string) {
    try {
      const res = await axios.get(`${ROOT_URL}/get-orders/${customerId}/${status}`, {
        headers: createHeaders(jwt_token)
      });
      return res.data;
    } catch (error) {
