...
const isNewFormat = ref<boolean>(false);

const updateApiCall = (payload: Record<string, string>): void => {
  isNewFormat.value = !payload.id;
  if (payload.id) {
...
const onSubmitEditItem = async(data: any) => {
  console.log('successfully added/edited a format !');
  console.log(data);
};
    ...
    >
      ...
      <template #form>
        <v-card class="text-center pa-5">
          <v-card-title>{{ isNewFormat ? 'Nouveau genre' : 'Modifier un genre' }}</v-card-title>
